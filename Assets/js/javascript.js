...
var apiKey = "a40c1489132a503eda1571ea2db577c9";
var cities=[];
var city_history=[];
var temps=[6];
var humid=[6];
...
function displayWeather(data) {
  ...
  for (let i=0;i<6;i++){
    ...
    dates[i] = new Date(data.list[index[i]].dt * 1000);
  }
  ...
  // Update the DOM elements with the relevant weather information
  for (let j=0; j<6; j++){
    tempEls[j].textContent = "Temperature: " + temps[j] + "°C";
    humidEls[j].textContent = "Humidity: " + humid[j] + "%";
    windEls[j].textContent = "Wind Speed: " + wind[j] + "KM/H";
...
// Load city history upon page load
printCityHistory();